...
export class SandSimulation {
  ...
  constructor(cols, rows, options = {}) {
    ...
    // Shake state — see startShake()/stopShake(). Purely a temporary
    // physics mode; it never adds, removes, or recolors grains, only
    // moves existing ones around.
    this._shaking = false;
    this._shakeBoost = 1;
  }
  ...
  /**
   * Start the shake physics mode: for `durationMs`-worth of steps (the
   * caller — Jar.shake() — controls timing and calls stopShake()), every
   * step() also runs a surface-flattening pass and slides more eagerly.
   * This only ever *moves* existing grains — never adds, removes, or
   * recolors any — so total sand and every color/layer is preserved.
   * @param {number} boost multiplier applied on top of the normal
   *   sandPhysicsIntensity while shaking (e.g. 1.6 = 60% more eager to slide)
   */
  startShake(boost = 1.6) {
    this._shaking = true;
    this._shakeBoost = boost;
  }
  ...
  /** Row index of the topmost filled cell in `col`, or `this.rows` if the column is empty. */
  _surfaceRow(col) {
    const { cols, rows, grid } = this;
    for (let row = 0; row < rows; row++) {
      if (grid[row * cols + col] !== 0) return row;
    }
    return rows;
  }

  /**
   * One pass of "vibration": for a handful of random adjacent column
   * pairs, if one column's pile is meaningfully taller than its neighbor,
   * hop a single grain from the tall column's peak over to the shorter
   * one. Repeated every step while shaking, this cascades tall peaks
   * sideways into a flatter, more settled-looking surface — exactly what
   * gently shaking a real jar of sand does. Grains are relocated, never
   * created or destroyed, so the total fill and every color stays intact.
   */
  _shakeSurfacePass() {
    const { cols, rows, grid } = this;
    const attempts = Math.max(6, Math.floor(cols * 0.18));

    for (let i = 0; i < attempts; i++) {
      const col = (Math.random() * cols) | 0;
      const dir = Math.random() < 0.5 ? -1 : 1;
      const neighbor = col + dir;
      if (neighbor < 0 || neighbor >= cols) continue;

      const topA = this._surfaceRow(col);
      if (topA === rows) continue; // column A is empty, nothing to move

      const topB = this._surfaceRow(neighbor);
      // Only hop if the neighbor's pile is at least 2 grains shorter —
      // avoids endlessly shuffling an already-flat surface back and forth.
      if (topB - topA < 2) continue;

      const destRow = topB - 1;
      const srcIdx = topA * cols + col;
      const destIdx = destRow * cols + neighbor;
      if (grid[destIdx] === 0 && grid[srcIdx] !== 0) {
        grid[destIdx] = grid[srcIdx];
        grid[srcIdx] = 0;
      }
    }
  }
  ...
  step() {
    this._dispensePours();
    if (this._shaking) this._shakeSurfacePass();

    const { cols, rows, grid } = this;
    this._stepParity ^= 1;
    const leftToRight = this._stepParity === 0;

    // While shaking, sand is more willing to slide off of a slope instead
    // of settling — the same probability the normal diagonal-settling
    // logic below uses, just temporarily boosted.
    const effectiveIntensity = this._shaking ? this.intensity * this._shakeBoost : this.intensity;
